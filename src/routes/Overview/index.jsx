  ...
  async componentDidMount() {
    try {
      const response = await get(
        `api/references?userEmail=${localStorage.getItem("email")}`
      );
      ...
      <div className="container" style={{ width: "80%" }}>
        <div className="row">
          ...
          >
            ...
            >
              {this.state.result[0] == [] ? (
                <Link to="/add-reference" style={{ width: "100%" }}>
                  <Button children="Share references" />
                </Link>
              ) : (
                <Link to="/shared-references" style={{ width: "100%" }}>
                  <Button children="Share references" />
                </Link>
              )}
              <Button as="a" href="/add-reference" style={{ width: "50%" }}>
                {" "}
                Add references
              </Button>
            </div>
